# Crear los eventos de Calendar — TRABAJO DE GRADO 3

**Grupos 54450, 54466, 54467** · martes 17:00–18:00 · **15 encuentros** · **112 estudiantes invitados** · 54450: 13 + 54466: 49 + 54467: 50

> **Archivo generado — no editar a mano.** Regenerar: `python config/slides/build_calendar_encuentros.py tg3`

## Qué vas a conseguir

Los **15 encuentros** del periodo (11/08/2026 → 17/11/2026) en tu Google Calendar, cada uno con **los tres grupos (54450, 54466, 54467)** en la sección *Invitados* y con **el mismo enlace de Google Meet** en todos. Unos cinco minutos. No hay que crear la sala a mano: la crea el propio script.

## ⚠️ Lo primero: no importes el `.ics`

Google Calendar **descarta la lista de invitados** al importar `.ics` y `.csv`. No es un defecto de estos archivos: es cómo Google trata esos formatos. Si importas el `.ics` de esta carpeta te quedan los 15 eventos **con cero de los 112 estudiantes**, y encima ya no puedes usar el script sin borrarlos antes.
...
En TG3 hay una trampa extra: esto es **una sola serie para los tres grupos**. Antes convivían carpetas `_combinado_54466-54467/` y `_combinado_todos_hasta_15-11/` con versiones alternativas de los mismos encuentros; se eliminaron precisamente porque importar dos de ellas creaba los eventos por duplicado. Lo único que crea encuentros de TG3 es el `.gs` de esta carpeta.
...
- las **15 sesiones**, con fecha, título del evento y cuántos invitados lleva cada una;
...
Crea los **15 eventos** con sus invitados y después pone la misma sala de Meet en todos. Tarda un poco: la creación de la sala es asíncrona y el script espera a que Google devuelva el enlace.
...
```
  ...
  | Es la de las 15 sesiones. Cópiala y pégala en el material:
  |   config/cursos/carga_academica_2026.json -> cursos.tg3.meet
  +--------------------------------------------------------------
```

**Hazlo.** Abre `config/cursos/carga_academica_2026.json`, busca `"tg3"` y pon esa URL en su campo `"meet"` (hoy está vacío). Después regenera el material:

```
python config/slides/build_pregrado_cursos.py --calendar-only
python config/slides/build_correo_bienvenida.py
python config/slides/build_calendar_encuentros.py tg3
```
...
- Publica el enlace en el aula de CDigital: https://cdigital.cun.edu.co/course/view.php?id=112321

## Llega matrícula nueva y los encuentros ya están creados

Pasa cada semestre: alguien se matricula tarde, o el aula tenía menos estudiantes que hoy. No hay que borrar ni rehacer los **15 encuentros** ni tocar el Meet. Hay dos órdenes nuevas para esto:
...
1. Descarga otra vez el listado del aula y sobrescribe `2026/54450/Listado estudiantes (CDigital).csv` (y el de los otros dos grupos).
2. `python config/slides/build_calendar_encuentros.py tg3` — el `.gs` sale con el roster nuevo dentro.
3. Pega el `PRINCIPAL - Crear encuentros con invitados (3 grupos).gs` actualizado en el proyecto de Apps Script (reemplaza el que había).
...
| Dice «creará UNA sala nueva» y **no querías** | Ya existía sala: pégala primero en `carga_academica_2026.json → cursos.tg3.meet`, regenera el `.gs` y vuelve a `verificar()`. |
...
- **`borrarEncuentros()`** — borra solo los eventos cuyo título coincide exactamente con los 15 de esta serie. No toca nada más de tu calendario. Si ya habías puesto `SEND_INVITES = true`, los estudiantes reciben la cancelación.
...
1. **La última sesión no es para todos.** S15, 17/11/2026 (martes), invita solo a **54466, 54467**: el grupo **54450** cierra el 15/11 y ese día ya no tiene clase. El `.gs` lo hace solo; se ve en `verificar()`, en la línea de esa fecha.
2. **Una sola sala de Meet para los tres grupos.** Es una sola serie, no tres. No crees salas por grupo ni ejecutes el script tres veces.

## Las sesiones que se van a crear

| # | Fecha | Tema | Invitados |
|---|---|---|---|
| 01 | 11/08/2026 (martes) | Presentación del curso · docente · estudiantes · ACAs | 112 |
| 02 | 18/08/2026 (martes) | Formulación de pregunta, objetivos y título | 112 |
| 03 | 25/08/2026 (martes) | Estructura del documento · taller de introducción | 112 |
| 04 | 01/09/2026 (martes) | Fase I de referentes de investigación | 112 |
| 05 | 08/09/2026 (martes) | Diseño de instrumento · desarrollo metodológico | 112 |
| 06 | 15/09/2026 (martes) | Comunidades de práctica y co-creación | 112 |
| 07 | 22/09/2026 (martes) | Experiencia creativa · análisis de datos | 112 |
| 08 | 29/09/2026 (martes) | Fase III de referentes · cierre del marco teórico | 112 |
| 09 | 06/10/2026 (martes) | Resultados, discusión y relación con referentes | 112 |
| 10 | 13/10/2026 (martes) | Resumen, palabras clave UNESCO, conclusiones y referencias | 112 |
| 11 | 20/10/2026 (martes) | Póster · evidencias · verificación antiplagio | 112 |
| 12 | 27/10/2026 (martes) | Sustentación ante jurados | 112 |
| 13 | 03/11/2026 (martes) | Entregables para repositorio institucional | 112 |
| 14 | 10/11/2026 (martes) | Ajustes finales · seguimiento post-sustentación | 112 |
| 15 | 17/11/2026 (martes) | Cierre administrativo · recepción de entregables | 99 |

Fechas y temas salen de `config/cursos/sesiones_cun.py`; el roster, de los listados de `2026/<grupo>/`; el enlace de Meet y el aula, de `config/cursos/carga_academica_2026.json`. Si cambia cualquiera de los tres, regenera este `.gs` con `python config/slides/build_calendar_encuentros.py tg3`.
