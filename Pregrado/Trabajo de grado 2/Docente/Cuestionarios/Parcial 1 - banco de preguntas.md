...
- Comando de validación usado (no toca el aula): `export PYTHONUTF8=1 && python config/moodle/cdigital.py importar "Pregrado/Trabajo de grado 2/Docente/Cuestionarios/Parcial 1 - banco de preguntas (Moodle XML).xml" --curso 129268 --simular` → *10 preguntas · validación local: sin problemas*.
...
5. ~~**Los decks de las Sesiones 02, 03 y 04 traen el marcador `[URL CDigital — campus del curso pendiente]`** en la diapositiva de trabajo autónomo.~~ **RESUELTO (2026-08-20).** El marcador ya no se escribe en el JSON de la sesión: ahora los bloques llevan `{{CDIGITAL}}` y `cun_contenido_sesion._resolver_aula()` lo sustituye en el build por lo que devuelve `sesiones_cun.cdigital_frase()` — para TG2, `https://cdigital.cun.edu.co/course/view.php?id=129268`. No hay que copiar la URL a mano en ninguna deck, y si el aula cambia de id no queda ninguna copia vieja.
