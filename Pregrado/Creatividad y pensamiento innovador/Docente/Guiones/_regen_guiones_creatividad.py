...
def _repo_root(desde: str) -> str:
    """Sube desde `desde` hasta el directorio que contiene `config/slides/`.

    Antes eran tres `..` contados a mano, correctos cuando este archivo vivía en
    `<Asignatura>/Guiones/`. Al mover la carpeta a `<Asignatura>/Docente/Guiones/` quedaron
    cortos y el regenerador murió con `ModuleNotFoundError: sesiones_cun`, que
    `build_sesion_material.py` traga como `WARN`: los guiones dejaron de actualizarse en
    silencio. Buscar hacia arriba sobrevive al próximo movimiento de carpeta.
    """
    d = desde
    while not os.path.isdir(os.path.join(d, "config", "slides")):
        padre = os.path.dirname(d)
        if padre == d:
            raise SystemExit(f"No encontré `config/slides/` subiendo desde {desde}")
        d = padre
    return d


REPO = _repo_root(ROOT)
SLIDES = os.path.join(REPO, "config", "slides")
CURSOS = os.path.join(REPO, "config", "cursos")
...
def slides_fase(label: str, *titulos: str) -> str:
    ...
    **Manda el .pptx, no el argumento.** Antes esta función buscaba el índice del título que le
    pasaba el llamador pero imprimía el argumento tal cual: si el deck cambiaba de redacción, el
    guion seguía diciéndole al Docente «la deck dice X» mientras la deck decía Y, y el único
    síntoma era un rango vacío que nadie mira. Costó una cita vencida de la S07 —«TALLER —
    Sustentación cruzada y consolidación (17 minutos)» cuando el título real ya no traía los
    minutos—. Ahora se cita el título real y lo que no aparezca en el deck queda en
    `DESAJUSTES_DECK` para que `main()` lo diga en voz alta.
    """
    reales = _deck_titulos(label) or []
    idx, mostrados = [], []
    for t in titulos:
        objetivo = _norm_titulo(t)
        pos = next((i for i, r in enumerate(reales) if _norm_titulo(r) == objetivo), None)
        if pos is None:
            # Redacción a medio actualizar: el título real es prefijo del citado (o al revés).
            pos = next(
                (
                    i
                    for i, r in enumerate(reales)
                    if _norm_titulo(r) and (
                        objetivo.startswith(_norm_titulo(r)) or _norm_titulo(r).startswith(objetivo)
                    )
                ),
                None,
            )
        if pos is None:
            DESAJUSTES_DECK.append(f"{label}: «{t}» no está en el deck")
            mostrados.append(t)
            continue
        if _norm_titulo(reales[pos]) != objetivo:
            # Sin flecha «→»: cp1252 no la tiene y este script imprime a la consola de Windows.
            DESAJUSTES_DECK.append(f"{label}: «{t}» -> se cita el real «{reales[pos]}»")
        idx.append(pos + 1)
        mostrados.append(reales[pos])
    rango = ""
    if idx:
        a, b = min(idx), max(idx)
        rango = f"**{a}**" if a == b else f"**{a}–{b}**"
    cuerpo = " → ".join(f"«{t}»" for t in mostrados)
    ...
    5: {
        ...
        "4️⃣": [
            ...
            ("s01_google_docs_inicio.png", "Docs — un solo documento con las dos mitades",
             "Títulos «A. Validación» y «B. Vigilancia»; se exporta a PDF y se guarda en el Drive "
             "del estudiante. Este avance **no se sube al aula**: la única tarea documental es la "
             "ACA Final, y este documento es uno de sus insumos."),
        ],
    },
    6: {
        "3️⃣": [
...
def inject_shots(md: str, n: int) -> str:
    ...
    if "Pantallazos de esta sesión" not in md and "Pantallazos en `Docente/Guiones/Capturas/`" not in md:
        md = md.replace(
...
**Slides:** 17–18 (PARA LA SESIÓN 02, la segunda es la «(cont.)») → 19–20 (PREGUNTAS DEL PRIMER DÍA) → 22 (PARA LA PRÓXIMA SESIÓN) → 23 (RUTA DE ENTREGABLES DEL CURSO) → 24 (Cierre)
...
✅ **Checklist del Docente antes de clase**
- [ ] Pantallazos de apoyo abiertos (`Docente/Guiones/Capturas/Sesion 01/`)
...
{slides_fase(label, "Errores frecuentes y respuestas", "Paso a paso: un solo documento con las dos mitades", "TALLER · Validar por dentro y por fuera", "TALLER · cómo se revisa y dónde se entrega")}
...
{slides_fase(label, "Errores frecuentes y respuestas", "TALLER · Mapa de entidades + guion del pitch", "TALLER · cómo se revisa y dónde se entrega")}
...
{slides_fase(label, "TALLER · Sustentación cruzada y consolidación", "TALLER · cómo se revisa y dónde se entrega")}

**GUION LITERAL (consigna):**
> “Pasamos al **TALLER**. Tienen **25 minutos** y cuatro pasos. Todo lo de hoy queda en `S07_AjustesCierre_Apellido` —Google Doc o PDF— y **se guarda en su Drive**: la ACA Final ya cerró la semana pasada, así que hoy no se sube ningún archivo al aula.”
...
def main(argv=None):
    ...
    **Sesión 01 = encuadre y se regenera siempre**, como cualquier otra sesión. La protección
    ...
    if DESAJUSTES_DECK:
        # Se dice, aunque el .md ya salió corregido: significa que una fase de este archivo
        # tiene la redacción del deck vieja y hay que ponerla al día aquí, no en el .md.
        print(f"\nDESAJUSTES contra el deck real: {len(DESAJUSTES_DECK)}")
        for d in DESAJUSTES_DECK:
            print("   · " + d)
