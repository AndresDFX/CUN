# Crear los eventos de Calendar — INVESTIGACIÓN, CIENCIA Y TECNOLOGÍA

**Grupo 53339** · jueves 17:00–18:00 · **6 encuentros** · **20 estudiantes invitados**

> **Archivo generado — no editar a mano.** Regenerar: `python config/slides/build_calendar_encuentros.py investigacion`

## Qué vas a conseguir

Los **6 encuentros** del periodo (13/08/2026 → 17/09/2026) en tu Google Calendar, cada uno con **los 20 estudiantes del grupo 53339** en la sección *Invitados* y con **el mismo enlace de Google Meet** en todos. Unos cinco minutos. No hay que crear la sala a mano: la crea el propio script.

## ⚠️ Lo primero: no importes el `.ics`

Google Calendar **descarta la lista de invitados** al importar `.ics` y `.csv`. No es un defecto de estos archivos: es cómo Google trata esos formatos. Si importas el `.ics` de esta carpeta te quedan los 6 eventos **con cero de los 20 estudiantes**, y encima ya no puedes usar el script sin borrarlos antes.
...
|---|---|
| `Calendario de clases - Grupo 53339.md` | Referencia: el cronograma en tabla. No se importa: se lee. |
...
| `RESPALDO sin invitados - Encuentros INVESTIGACIÓN, CIENCIA Y TECNOLOGÍA - Grupo 53339.csv` | ⚠️ **Respaldo de fechas. NO importar** salvo emergencia: Google descarta los invitados al importar `.ics`/`.csv`. |
| `RESPALDO sin invitados - Encuentros INVESTIGACIÓN, CIENCIA Y TECNOLOGÍA - Grupo 53339.ics` | ⚠️ **Respaldo de fechas. NO importar** salvo emergencia: Google descarta los invitados al importar `.ics`/`.csv`. |

## Paso a paso

### 1. Abre Apps Script con la cuenta CUN

**https://script.google.com** con **julian_castanoe@cun.edu.co**. Si entras con otra cuenta, los eventos se crean en el calendario equivocado. **Nuevo proyecto** → borra el `function myFunction()` que trae de fábrica → pega **todo** el contenido de `PRINCIPAL - Crear encuentros con invitados.gs` → guarda. Ponle al proyecto un nombre reconocible (p. ej. «Encuentros Investigación, ciencia y tecnología 2026»).
...
- las **6 sesiones**, con fecha, título del evento y cuántos invitados lleva cada una;
...
Crea los **6 eventos** con sus invitados y después pone la misma sala de Meet en todos. Tarda un poco: la creación de la sala es asíncrona y el script espera a que Google devuelva el enlace.
...
```
  ...
  | Es la de las 6 sesiones. Cópiala y pégala en el material:
  |   config/cursos/carga_academica_2026.json -> cursos.investigacion.meet
  +--------------------------------------------------------------
```

**Hazlo.** Abre `config/cursos/carga_academica_2026.json`, busca `"investigacion"` y pon esa URL en su campo `"meet"` (hoy está vacío). Después regenera el material:

```
python config/slides/build_pregrado_cursos.py --calendar-only
python config/slides/build_correo_bienvenida.py
python config/slides/build_calendar_encuentros.py investigacion
```
...
- Publica el enlace en el aula de CDigital: https://cdigital.cun.edu.co/course/view.php?id=111070

## Llega matrícula nueva y los encuentros ya están creados

Pasa cada semestre: alguien se matricula tarde, o el aula tenía menos estudiantes que hoy. No hay que borrar ni rehacer los **6 encuentros** ni tocar el Meet. Hay dos órdenes nuevas para esto:
...
1. Descarga otra vez el listado del aula y sobrescribe `2026/53339/Listado estudiantes (CDigital).csv`.
2. `python config/slides/build_calendar_encuentros.py investigacion` — el `.gs` sale con el roster nuevo dentro.
...
| Dice «creará UNA sala nueva» y **no querías** | Ya existía sala: pégala primero en `carga_academica_2026.json → cursos.investigacion.meet`, regenera el `.gs` y vuelve a `verificar()`. |
...
- **`borrarEncuentros()`** — borra solo los eventos cuyo título coincide exactamente con los 6 de esta serie. No toca nada más de tu calendario. Si ya habías puesto `SEND_INVITES = true`, los estudiantes reciben la cancelación.
...
|---|---|---|---|
| 01 | 13/08/2026 (jueves) | Presentación del curso · docente · estudiantes · ACAs | 20 |
| 02 | 20/08/2026 (jueves) | MinCiencias · 6 líneas de Ingeniería · elección de línea | 20 |
| 03 | 27/08/2026 (jueves) | Prueba parcial · 1.er avance del artículo | 20 |
| 04 | 03/09/2026 (jueves) | Problema y pregunta · bases de datos y gestores de citas | 20 |
| 05 | 10/09/2026 (jueves) | Planteamiento del problema · marco teórico y revisión de literatura | 20 |
| 06 | 17/09/2026 (jueves) | Socialización del artículo y cierre del curso | 20 |

Fechas y temas salen de `config/cursos/sesiones_cun.py`; el roster, de los listados de `2026/<grupo>/`; el enlace de Meet y el aula, de `config/cursos/carga_academica_2026.json`. Si cambia cualquiera de los tres, regenera este `.gs` con `python config/slides/build_calendar_encuentros.py investigacion`.
