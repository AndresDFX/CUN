...
| `no hay ningún encuentro en el Calendar …` | Lo más probable: **la serie de encuentros de ese curso todavía no existe**. Créala con `PRINCIPAL - Crear encuentros con invitados.gs` del curso (necesita la sala de Meet en `carga_academica_2026.json → cursos.<key>.meet`) y ejecuta `reintentarPendientes()`. Si la serie ya existe y aun así sale esto, era una tutoría, un jurado o una reunión ajena: **no** debe publicarse. |
