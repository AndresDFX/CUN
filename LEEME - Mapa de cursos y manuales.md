...
- **Eventos (postulaciones a congresos y encuentros)** — `Eventos/<Nombre del evento>/`, carpeta raíz creada el **2026-08-27**. Una subcarpeta por evento con cuatro archivos: el `.md` del resumen o memoria (**fuente de verdad**), el `.docx` generado desde él (lo que se sube), `HOJA_DE_RESPUESTAS_Formulario.md` (campo por campo, para copiar y pegar, con las decisiones que solo puede tomar el Docente marcadas como pendientes) y `_armar_resumen_docx.py`, el constructor **de ese evento**. El texto se edita en el `.md` y se regenera; nunca en Word. **No usar `config/slides/guion_md_a_docx.py`**: dimensionado para guiones y fichas de `Clases/`, derrama un resumen de cuatro párrafos en dos hojas. El número de páginas se mide exportando a PDF con `soffice --headless`, no a ojo. Un evento **no** paga el producto `ART_OPEN_D` de Synapse: va al de *Evento científico*, y la evidencia es el certificado con el rol ejercido. Agente: `publicaciones-cun` §9 bis. Primera carpeta: `XIII Semana de la Investigacion 2026/` (CUN, resumen de 282 palabras sobre examlab, rol Expositor/Obra).
